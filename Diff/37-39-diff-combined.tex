...
%DIF DEL Coroutines-SMS-37-combined.tex   Tue Feb 12 10:30:37 2019
%DIF ADD Coroutines-SMS-39-combined.tex   Tue Feb 19 08:10:21 2019
...
\DIFdelbegin %DIFDELCMD < \title[A survey of asynchronous programming using coroutines in IoT and embedded systems]{A survey of asynchronous programming using coroutines in the Internet of Things and embedded systems}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \title[A Survey of Asynchronous Programming Using Coroutines in IoT and Embedded Systems]{A Survey of Asynchronous Programming Using Coroutines in the Internet of Things and Embedded Systems}
\DIFaddend 
...
%DIFDELCMD < 	\institution{James Cook University}
%DIFDELCMD < 	\department{College of Business, Law \& Governance}
%DIFDELCMD < 	%\city{Cairns}
%DIFDELCMD < 	%\state{Queensland}
%DIFDELCMD < 	%\postcode{4870}
%DIFDELCMD < 	%\country{Australia}
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFaddend \email{jason.holdsworth@jcu.edu.au}
...
The Internet of Things (IoT) \cite{Al-Fuqaha2015, Atzori2010, Gubbi2013} continues to grow both in the scale and variety of attached devices and in the number of developed applications \cite{Manyika2015, VanderMeulen2017}. This growth draws attention to the software engineering of the resource-constrained embedded systems that are a frequent component of heterogeneous IoT applications\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. Such attention is all the more urgently required }\DIFaddend because of new challenges with regard to security \cite{Sicari2015}, reliability \cite{Gubbi2013} and privacy \cite{Weber2015}.

Many IoT and embedded systems \DIFdelbegin \DIFdel{software }\DIFdelend have an event-driven architecture\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{; their software is consequently implemented in }\DIFaddend an asynchronous programming style, whereby multiple tasks wait on external events. Asynchronous code is challenging to write because application logic becomes split between the function initiating the request and the event handler that is invoked when the response is ready \cite{Gay2003, Levis2002, Meijer2010}. This “split-phase” architecture becomes increasingly complex when the developer introduces more event sources (such as timeouts) with their own event handlers. \DIFaddbegin \DIFadd{There may be interaction between various split-phase events, which can add degrees of freedom to the various state models: consequently there is an increasing likelihood that the source code addressing a single event is split between separate locations, forcing the reader to jump between them. }\DIFaddend Application logic is obscured by the split-phase fragmentation, leading to a gap between the design of the system and its source code representation, making the code harder to understand and more difficult to maintain \cite{Brodu2015, Edwards2009, Madsen2017, Kambona2013}.

A solution to the split-phase problem for desktop software has been language support for coroutines \cite{Conway1963, Knuth1968, Marlin1979} and promises \cite{Brodu2015, Liskov1988, Madsen2017}. For example, in C\#, JavaScript, and Python, developers can use an “await” keyword to wait on an external event. This means that asynchronous code can be written just as clearly as the equivalent code in a synchronous style that uses blocking code. However, resource-constrained embedded systems are overwhelmingly programmed in C or \DIFdelbegin %
{\color{red}%DIFAUXCMD
\verb!C++! %DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \verb!C++! %%%
\DIFdelend \DIFaddbegin \DIFadd{C++ }\DIFaddend \cite{AspenCore2017, Skerrett2017}, which lack support for the “await” pattern.

The C++ standardisation committee is currently debating the inclusion of coroutines, and at least two competing designs have been proposed \cite{ISO2017, Romer2018}. The addition of coroutines to C++ would create an opportunity to simplify embedded systems code. \DIFdelbegin %DIFDELCMD < 
...
The remainder of this paper is organised as follows. Section \ref{section:background} contains the background\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{, beginning with }\DIFaddend an introduction to the development environment for C/C++ programs on resource-constrained devices, to some of the problems commonly encountered by developers and to the types of solution currently applied to these problems\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{. It continues with }\DIFaddend a discussion of the use of coroutines in C and C++\DIFdelbegin \DIFdel{; and a review of related work}\DIFdelend . Section \ref{section:sms} details the methodology of the mapping process used in this study, \DIFdelbegin \DIFdel{and }\DIFdelend some of the logic underpinning the methodological choices\DIFaddbegin \DIFadd{, and a review of related work}\DIFaddend . Section \ref{section:results} explores the results and presents insights. Section \ref{section:analysis} contains a discussion of results and an analysis of research gaps. Section \ref{section:conclusion} discusses future research possibilities and concludes the paper.


\DIFdelbegin %DIFDELCMD < \section{Background and related work}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \section{Background}
\DIFaddend \label{section:background}
...
Coroutine implementations may be further categorised into stackful or stackless types. A stackful coroutine has its own stack \DIFaddbegin \DIFadd{which is not shared with the caller}\DIFaddend , and hence local variables can be stored there during suspension. Conversely, a stackless coroutine pops its state off the stack during suspension (like a normal function return). \DIFdelbegin \DIFdel{Neither model is considered universally appropriate for the various C++ use cases \mbox{%DIFAUXCMD
\cite{Goodspeed2014, Riegel2015}}\hspace{0pt}%DIFAUXCMD
. }\DIFdelend For stackless coroutines, other mechanisms must be introduced in order to preserve state\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, such as storing local variables in global storage. 
}

\DIFaddend Furthermore, a stackless coroutine \DIFdelbegin \DIFdel{cannot }\DIFdelend \DIFaddbegin \DIFadd{often can only }\DIFaddend be suspended from \DIFdelbegin \DIFdel{a subroutine; the yield must take place in }\DIFdelend \DIFaddbegin \DIFadd{within the coroutine itself and not from a subroutine (i.e. a function called from the coroutine). For example, C++ proposal N4680 is a stackless model that requires all yield or return statements to be contained within }\DIFaddend the body of the coroutine\DIFdelbegin \DIFdel{itself.
}\DIFdelend \DIFaddbegin \DIFadd{.
}

\DIFadd{Neither model is considered universally appropriate for the various C++ use cases \mbox{%DIFAUXCMD
\cite{Goodspeed2014, Riegel2015}}\hspace{0pt}%DIFAUXCMD
...
\DIFdelbegin \DIFdel{In 2000, Tatham \mbox{%DIFAUXCMD
\cite{Tatham2000} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Early implementations used macros in C to add coroutine-like features. For example, Duff’s device takes advantage of the fall-through behaviour of C’s case statement in the absence of a break statement \mbox{%DIFAUXCMD
\cite{Duff88}}\hspace{0pt}%DIFAUXCMD
. It is unusual in that a block such as do … while, can be interleaved within the case statements of a switch statement. \mbox{%DIFAUXCMD
\citeN{Tatham2000} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend described a coroutine solution in C, which makes use of Duff’s device \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Duff88} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend to efficiently implement coroutines through macros, without the need to explicitly code a state machine\DIFdelbegin \DIFdel{, and thereby satisfies the producer-consumer pattern}\DIFdelend \DIFaddbegin \DIFadd{. However, Tatham noted that “this trick violates every coding standard in the book” and Duff called the method a “revolting way to use switches to implement interrupt driven state machines”}\DIFaddend . This technique was extended by Dunkels et al. for Protothreads \cite{Dunkels2006}, which provided conditional blocking operations on memory-constrained systems, without the need for multiple stacks, and formed the core of the widely used real-time operating system Contiki \cite{Dunkels2004}.
...
Listing \ref{protothreads} contains a fragment of code that used Protothreads to implement part of an asynchronous producer/consumer pattern. Listing \ref{coawait} shows a similar code fragment, this time using C\DIFdelbegin %DIFDELCMD < {}\verb!++! %%%
\DIFdelend \DIFaddbegin \DIFadd{++ }\DIFaddend language features, including the co\_await keyword of the current C\DIFdelbegin %DIFDELCMD < {}\verb!++! %%%
\DIFdelend \DIFaddbegin \DIFadd{++ }\DIFaddend standardisation proposal N4680. \DIFaddbegin \DIFadd{We observe several differences between the two. Listing \ref{coawait} contains fewer lines of code than Listing \ref{protothreads}; Listing \ref{coawait} does not contain macros; Listing \ref{protothreads} requires that local variables be declared as ‘static’, but Listing \ref{coawait} does not. While both code fragments present conceptual changes from the synchronous equivalent, we believe that the change in Listing \ref{coawait} is more transparent and more clearly presented.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{lstlisting}%DIFDELCMD < [language=C, caption=Fragment of Protothreads code for asynchronous producer/consumer threads, label=protothreads]
%DIFDELCMD < static struct pt_sem full, empty;
%DIFDELCMD < 

%DIFDELCMD < static
%DIFDELCMD < PT_THREAD(consumer(struct pt *pt))
%DIFDELCMD < {
%DIFDELCMD < static int consumed;
%DIFDELCMD < PT_BEGIN(pt);
%DIFDELCMD < for (consumed = 0; consumed < NUM_ITEMS; 
%DIFDELCMD < ++consumed) {
%DIFDELCMD < PT_SEM_WAIT(pt, &empty);
%DIFDELCMD < consume_item(get_from_buffer());
%DIFDELCMD < PT_SEM_SIGNAL(pt, &full);
%DIFDELCMD < }
%DIFDELCMD < PT_END(pt);
%DIFDELCMD < }
%DIFDELCMD < \end{lstlisting}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{lstlisting}[language=C, caption=Fragment of Protothreads code for asynchronous producer/consumer threads, label=protothreads]
...
\DIFdelbegin %DIFDELCMD < \begin{lstlisting}%DIFDELCMD < [language=C++, caption=C++ code fragment using co\_await for asynchronous producer/consumer threads, label=coawait]
%DIFDELCMD < task<> consumer(semaphore& sem) {
%DIFDELCMD < auto producer = async_producer(sem, NUM_ITEMS);
%DIFDELCMD < for co_await(auto consumed : producer) {
%DIFDELCMD < consume_item(get_fom_buffer());
%DIFDELCMD < }
%DIFDELCMD < }
%DIFDELCMD < \end{lstlisting}
%DIFDELCMD < %%%
...
\DIFdelbegin \DIFdel{Schimpf \mbox{%DIFAUXCMD
\cite{Schimpf2012b} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{In 1992, Gupta et al. examined a coroutine-based concurrency model for resource-constrained platforms as part of a comparison between alternative models \mbox{%DIFAUXCMD
\cite{Gupta1992}}\hspace{0pt}%DIFAUXCMD
. In 2000, Engelschall summarised the various techniques based on setjmp \& longjmp \mbox{%DIFAUXCMD
\cite{Engelschall2000}}\hspace{0pt}%DIFAUXCMD
. FreeRTOS \mbox{%DIFAUXCMD
\cite{Barry2018} }\hspace{0pt}%DIFAUXCMD
is an open source real-time operating system developed "around 2003" that contains a coroutine scheduler: local variable state is not maintained. In 2006, Rossetto and Rodriguez described a new concurrency model \mbox{%DIFAUXCMD
\cite{Rossetto2006} }\hspace{0pt}%DIFAUXCMD
implemented as an extension to TinyOS \mbox{%DIFAUXCMD
\cite{Levis2005}}\hspace{0pt}%DIFAUXCMD
, using coroutines as the basis of the integration; the implementation is stackful and local variables’ states are maintained. 
}

%DIF >  Kowalke’s Boost Coroutine C++ library \cite{Kowalke2009} is stackful; the second version \cite{Kowalke2014} has support for a variety of different strategies for stack allocation.

\DIFadd{\mbox{%DIFAUXCMD
\citeN{Schimpf2012b} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend provides a modified version of Protothreads which supports a priority-based scheduler. \DIFdelbegin \DIFdel{Cohen et al. \mbox{%DIFAUXCMD
\cite{Cohen2007b} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeN{Cohen2007b} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend provide a coroutine-based scheduler for TinyOS \cite{Levis2005} which is used to implement “RPC-like interfaces”; these support a direct programming style for communications code written in nesC \cite{Gay2003}. \DIFdelbegin \DIFdel{Riedel et al.\mbox{%DIFAUXCMD
\cite{Riedel2010} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeN{Riedel2010} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend generate C code for multiple platforms, including a version that uses coroutines to provide concurrency. \DIFdelbegin \DIFdel{Susilo et al. \mbox{%DIFAUXCMD
\cite{Susilo2009} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeN{Susilo2009} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend use a coroutine-based scheduler to achieve “[r]eal time multitasking […] without interrupts”. Finally, \DIFdelbegin \DIFdel{Andersen et al. \mbox{%DIFAUXCMD
\cite{Andersen2017b} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeN{Andersen2017b} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend reject the use of C++ futures because the implementation model needs to handle a stream of events, rather than a single event, and is therefore non-deterministic in its use of memory, which is undesirable on a constrained platform: “One is forced therefore to trade off the reliability of promises […] in order for them to work in the embedded space.” Instead, the authors use callbacks for C++ event handling.
...
\DIFdelbegin \DIFdel{The search, }\DIFdelend \DIFaddbegin \DIFadd{Two searches were conducted in October 2017 and in September 2018 }\DIFaddend across all databases\DIFdelbegin \DIFdel{, found }\DIFdelend \DIFaddbegin \DIFadd{. These searches resulted in }\DIFaddend 187 journal articles, 224 conference papers and 155 book chapters. This informed our decision to include all three publication classes within the search domain. The decision was made to include only studies published since 2007; this criterion excluded approximately 43\% of the original search results.
...
RQ1 investigated the software platform, including \DIFdelbegin \DIFdel{programing language, }\DIFdelend \DIFaddbegin \DIFadd{the programming language, the }\DIFaddend operating system and the implementation used for the relevant language feature. RQ2 looked at the hardware platform, including memory size and processor family. RQ3 and RQ4 assessed the use cases and intended benefits respectively of the coroutine usage. RQ5 assessed the programming interface.

The research questions are listed in full in \DIFdelbegin \DIFdel{the supplementary materials. }\DIFdelend \DIFaddbegin \DIFadd{Table \ref{RQs}. By examining the hardware and software characteristics of previous implementations we aimed to identify the salient characteristics of the environment within which a coroutine implementation must function. By investigating use cases and desired outcomes, we would identify some of the necessary characteristics of a successful implementation. Finally, by examining the programming interface we hoped to observe how researchers addressed some of the design trade-offs of the implementation.
}\DIFaddend 

\DIFaddbegin \begin{table}[h]
	\caption{\DIFaddFL{Research questions}}
	\label{RQs}

	\begin{tabular}{l l l}
		\hline
		\DIFaddFL{Code }& & \DIFaddFL{Research question }\\
		\hline
		\DIFaddFL{RQ1	}& & \DIFaddFL{What was the software platform?}\\
		& \DIFaddFL{RQ1a }& \DIFaddFL{What was the programming language used?}\\
		& \DIFaddFL{RQ1b }& \DIFaddFL{What method was used to implement coroutines?}\\
		& \DIFaddFL{RQ1c }& \DIFaddFL{What was the operating system used (if any)?}\\
		\DIFaddFL{RQ2	}& & \DIFaddFL{What was the hardware platform?}\\
		& \DIFaddFL{RQ2a }& \DIFaddFL{What was the class of hardware platform?}\\
		& \DIFaddFL{RQ2b }& \DIFaddFL{How much read-only or flash memory (ROM) was available?}\\
		& \DIFaddFL{RQ2c }& \DIFaddFL{How much random-access memory (RAM) was available?}\\
		& \DIFaddFL{RQ2d }& \DIFaddFL{What was the processor family?}\\
		& \DIFaddFL{RQ2e }& \DIFaddFL{Was the processor 8-bit, 16-bit or 32-bit?}\\
		& \DIFaddFL{RQ2f }& \DIFaddFL{What was the processor’s instruction set?}\\
		\DIFaddFL{RQ3	 }&  & \DIFaddFL{What were the use cases?}\\
		\DIFaddFL{RQ4	 }&  & \DIFaddFL{What were the intended benefits of using coroutines in this context?}\\
		\DIFaddFL{RQ5	 }&  & \DIFaddFL{What is the API of the coroutine?}\\
		& \DIFaddFL{RQ5a }& \DIFaddFL{Does the paper discuss an implementation of coroutines?}\\
		& \DIFaddFL{RQ5b }& \DIFaddFL{Is the control flow managed on behalf of the developer?}\\
		& \DIFaddFL{RQ5c }& \DIFaddFL{Is the state of local variables automatically managed?}\\
		& \DIFaddFL{RQ5d }& \DIFaddFL{Is the coroutine implementation stackless or stackful?}\\
		& \DIFaddFL{RQ5e }& \DIFaddFL{How is the coroutine state allocated?}\\
		\hline
	\end{tabular}
\end{table}

\DIFaddend \subsection{Threats to validity}
...
During study selection, the search process was recorded in detail and the search strings were tested for repeatability and for consistency across databases. \DIFaddbegin \DIFadd{Snowballing describes the process of expanding the search results by recursively selecting papers cited by a selected paper or which cite a selected paper \mbox{%DIFAUXCMD
\cite{Kitchenham2011a, Petersen2015}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend While the study did not utilise snowballing \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Kitchenham2011a, Petersen2015} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend during the final search process, it did use \DIFdelbegin \DIFdel{snowballing }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend during the earlier stages of establishing search strings, and some searches were consequently amended. During the application of selection criteria, the reviewers \DIFdelbegin \DIFdel{consulted }\DIFdelend \DIFaddbegin \DIFadd{conferred }\DIFaddend whenever differences arose, and periodically discussed and reviewed the processes being used, using both contentious cases and randomly selected test papers to compare individual processes.
...
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\begin{subfigure}[h]{0.48\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics[width=\textwidth]{RQ1a-Programming-languages-used}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{RQ1a - Programming languages used}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:rq1a}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	\begin{subfigure}[h]{0.46\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics[width=\textwidth]{RQ1b-Coroutine-implementation}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{RQ1b - Coroutine implementation}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:rq1b}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
\DIFaddbegin \begin{figure}[h]
	...
	\begin{subfigure}[h]{0.48\textwidth}
		...
		\caption{\DIFaddFL{RQ1a - Programming languages used}}
		\label{fig:rq1a}
	\end{subfigure}
	\begin{subfigure}[h]{0.46\textwidth}
		\centering
		\includegraphics[width=\textwidth]{RQ1b-Coroutine-implementation}
		\caption{\DIFaddFL{RQ1b - Coroutine implementation}}
		\label{fig:rq1b}
	\end{subfigure}
	\caption{\DIFaddFL{Language outcomes}}
\end{figure}

\DIFaddend \subsection{Coroutine implementation}

To implement coroutines, 27 papers (77\%) used a native method, i.e. avoiding techniques that required a new or changed \DIFdelbegin \DIFdel{tool-chain}\DIFdelend \DIFaddbegin \DIFadd{tool chain}\DIFaddend . In native implementations, \DIFdelbegin \DIFdel{macros (}\DIFdelend 13 papers \DIFdelbegin \DIFdel{) and libraries (}\DIFdelend \DIFaddbegin \DIFadd{employed macros (of which 7 were based on Duff’s device) and }\DIFaddend 4 \DIFdelbegin \DIFdel{papers) were common}\DIFdelend \DIFaddbegin \DIFadd{used libraries}\DIFaddend ; in 3 papers (\cite{Yu2008, Cohen2007b, Kalebe2017}) the C setjmp/longjmp language device was used.

Several studies extended the \DIFdelbegin \DIFdel{tool-chain }\DIFdelend \DIFaddbegin \DIFadd{tool chain }\DIFaddend or created a new tool. \DIFdelbegin \DIFdel{2 }\DIFdelend \DIFaddbegin \DIFadd{Two }\DIFaddend papers contributed new languages \cite{Jahier2016, Evers2007}, and one paper \cite{Niebert2014} provided a set of language extensions. \DIFdelbegin \DIFdel{2 papers \mbox{%DIFAUXCMD
\cite{Jahier2016, Karpin2007} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Two papers }\DIFaddend employed a transpiler \DIFdelbegin \DIFdel{, one }\DIFdelend \DIFaddbegin \DIFadd{that translates from one language to another - one from Lustre to OCaml \mbox{%DIFAUXCMD
\cite{Jahier2016} }\hspace{0pt}%DIFAUXCMD
and one from a synchronous extension of C to standard C \mbox{%DIFAUXCMD
\cite{Karpin2007}}\hspace{0pt}%DIFAUXCMD
. One paper }\DIFaddend \cite{Fritzsche2010} used a precompiler, and one paper \cite{Jaaskelainen2008} provided a new compiler optimisation phase.
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < 	\includegraphics[width=0.5\textwidth]{RQ1c-Operating-systems-used-in-selected-c-like-studies}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{RQ1c - Operating systems used in selected studies using C-like languages}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:rq1c}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
\DIFdelbegin %DIFDELCMD < \subsection{Memory}
%DIFDELCMD < 
...
%DIFDELCMD < 	\begin{subfigure}[h]{0.46\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics[width=\textwidth]{RQ2b-c-Memory}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.5\textwidth]{RQ1c-Operating-systems-used-in-selected-c-like-studies}
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{RQ2b/c }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{RQ1c }\DIFaddendFL - \DIFdelbeginFL \DIFdelFL{Memory}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Operating systems used in selected studies using C-like languages}\DIFaddendFL }
	\DIFdelbeginFL %DIFDELCMD < \label{fig:rq2bc}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	\begin{subfigure}[h]{0.46\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics[width=\textwidth]{RQ2d-Processor-families-arranged-by-bits}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{RQ2d - Processor families arranged by bits}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:rq2d}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Hardware outcomes}}
%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \label{fig:rq1c}
\DIFaddendFL \end{figure}

\DIFaddbegin \subsection{Memory}
...
\DIFaddbegin \begin{figure}[h]
	...
	\begin{subfigure}[h]{0.46\textwidth}
		...
		\caption{\DIFaddFL{RQ2b/c - Memory}}
		\label{fig:rq2bc}
	\end{subfigure}
	\begin{subfigure}[h]{0.46\textwidth}
		\centering
		\includegraphics[width=\textwidth]{RQ2d-Processor-families-arranged-by-bits}
		\caption{\DIFaddFL{RQ2d - Processor families arranged by bits}}
		\label{fig:rq2d}
	\end{subfigure}
	\caption{\DIFaddFL{Hardware outcomes}}
\end{figure}
...
It is also notable that within the 8-bit segment, all but one were of the megaAVR family; among 16-bit processors 5 out of 7 used the TI MSP430 architecture. Within the 32-bit segment the picture was less clear-cut: just over half used the ARM architecture. These \DIFaddbegin \DIFadd{types of microcontrollers are widespread in IoT and embedded systems \mbox{%DIFAUXCMD
\cite{AspenCore2017}}\hspace{0pt}%DIFAUXCMD
. These }\DIFaddend results are summarised in Figure \ref{fig:rq2d}. Full details are in the supplementary materials.
...
%DIFDELCMD < 	\begin{subfigure}[h]{0.46\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics[width=\textwidth]{RQ3-Use-cases}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{RQ3 - Use cases}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:rq3}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	\begin{subfigure}[h]{0.46\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics[width=\textwidth]{RQ4-Intended-benefits}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{RQ4 - Intended benefits}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:rq4}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Usage outcomes}}
%DIFAUXCMD
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
\DIFaddbegin \begin{figure}[h]
	...
	\begin{subfigure}[h]{0.46\textwidth}
		...
		\caption{\DIFaddFL{RQ3 - Use cases}}
		\label{fig:rq3}
	\end{subfigure}
	\begin{subfigure}[h]{0.46\textwidth}
		\centering
		\includegraphics[width=\textwidth]{RQ4-Intended-benefits}
		\caption{\DIFaddFL{RQ4 - Intended benefits}}
		\label{fig:rq4}
	\end{subfigure}
	\caption{\DIFaddFL{Usage outcomes}}
\end{figure}
...
This paper has suggested that the management of control flow on behalf of the programmer (RQ5b) is a desirable feature of programming languages on resource-constrained platforms. The results in Figure \ref{fig:rq5bcd} appear to support this claim. All non-native C and almost all non-C implementations provide support for managing control flow. (The only exception is \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Motika2015}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{found in the work of \mbox{%DIFAUXCMD
\citeN{Motika2015}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , a pattern whose code is primarily designed as a target for code generators.) Additionally, 86\% of the native C cases were able to provide this feature, primarily through macros.
...
None of the native C implementations and only \DIFdelbegin \DIFdel{1 }\DIFdelend \DIFaddbegin \DIFadd{one }\DIFaddend of the non-native C implementations were stackful\DIFdelbegin \DIFdel{; in contrast}\DIFdelend \DIFaddbegin \DIFadd{. By contrast, }\DIFaddend 82\% of the non-C implementations were stackful. It could be argued that this split indicates that\DIFaddbegin \DIFadd{, while }\DIFaddend stackfulness is a desirable feature for language designers \DIFdelbegin \DIFdel{. However}\DIFdelend \DIFaddbegin \DIFadd{in general, it is less desirable for C developers. Our interpretation is that}\DIFaddend , because of the perceived costs of stackfulness in terms of memory and speed, there remains strong support in the C/C++ developer community for stackless coroutines \cite{Dunkels2006}.

The allocation of coroutine state is an important feature of the design with regard to its effect on resource-constrained platforms, since it must be controlled carefully if the design is to offer predictable and safe behaviour. Of the 16 implementations where \DIFaddbegin \DIFadd{we were able to determine }\DIFaddend the allocation method\DIFdelbegin \DIFdel{was determinable}\DIFdelend , nearly a third used an object or structure to store the state. 44\% (7 instances) required that the state be allocated in static (global) memory; 1 used only the stack, and 3 offered flexibility as regards the location.
...
Our survey indicates that multiple studies exist that require a coroutine-based facility for concurrent programming on resource-constrained devices, establishing that a demand exists at this end of the spectrum, not merely on high performance platforms. We noted in \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Section }\DIFaddend \ref{analysis-of-api-design} that, where the language allowed fine-tuned management of memory allocation, dynamic allocation of memory was avoided for coroutine state and stack. We can conclude that avoiding heap memory is a requirement for the small devices that formed the bulk of the target platforms.

While 82\% of non-C implementations \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend stackful, only one of the C implementations \DIFdelbegin \DIFdel{was stackful: it can be concluded that a }\DIFdelend \DIFaddbegin \DIFadd{is stackful. We observe that when a language is designed from the ground up to support coroutines, then a stackful implementation is common. On the other hand, such a feature is difficult in C, while preserving both backward compatibility and acceptable memory usage. We conclude that a }\DIFaddend stackless implementation is important to C programmers, and this reflects the scarcity of memory resources on the platforms under consideration.
