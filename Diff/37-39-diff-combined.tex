...
%DIF ADD Coroutines-SMS-39-combined.tex   Mon Feb 18 18:22:50 2019
...
While 82\% of non-C implementations \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend stackful, only one of the C implementations \DIFdelbegin \DIFdel{was stackful: it can be concluded }\DIFdelend \DIFaddbegin \DIFadd{is stackful. We observe that when a language is designed from the ground up to support coroutines, then a stackful implementation is common. On the other hand, such a feature is difficult in C, while preserving both backward compatibility and acceptable memory usage. We conclude }\DIFaddend that a stackless implementation is important to C programmers, and this reflects the scarcity of memory resources on the platforms under consideration.
