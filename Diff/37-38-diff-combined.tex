...
%DIF ADD Coroutines-SMS-38-combined.tex   Thu Feb 14 12:38:13 2019
...
. }\DIFdelend For stackless coroutines, other mechanisms must be introduced in order to preserve state\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, such as storing local variable in global storage. 
}

\DIFaddend Furthermore, a stackless coroutine \DIFdelbegin \DIFdel{cannot }\DIFdelend \DIFaddbegin \DIFadd{often can only }\DIFaddend be suspended from \DIFdelbegin \DIFdel{a subroutine; the yield must take place in }\DIFdelend \DIFaddbegin \DIFadd{within the coroutine itself and not from a subroutine (a function called from the coroutine). For example, C++ proposal N4680 is a stackless model that requires all yield or return statements to be contained within }\DIFaddend the body of the coroutine\DIFdelbegin \DIFdel{itself.
...
. It is unusual in that a block such as do … while, can be interleaved within the case statements of a switch statement. Tatham }\DIFaddend described a coroutine solution in C, which makes use of Duff’s device \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Duff88} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend to efficiently implement coroutines through macros, without the need to explicitly code a state machine\DIFdelbegin \DIFdel{, and thereby satisfies the producer-consumer pattern}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Tatham2000}}\hspace{0pt}%DIFAUXCMD
. However, Tatham noted that “this trick violates every coding standard in the book” and Duff called the method a “revolting way to use switches to implement interrupt driven state machines”}\DIFaddend . This technique was extended by Dunkels et al. for Protothreads \cite{Dunkels2006}, which provided conditional blocking operations on memory-constrained systems, without the need for multiple stacks, and formed the core of the widely used real-time operating system Contiki \cite{Dunkels2004}.
...
Several studies extended the \DIFdelbegin \DIFdel{tool-chain }\DIFdelend \DIFaddbegin \DIFadd{tool chain }\DIFaddend or created a new tool. 2 papers contributed new languages \cite{Jahier2016, Evers2007}, and one paper \cite{Niebert2014} provided a set of language extensions. 2 papers \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Jahier2016, Karpin2007} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend employed a transpiler \DIFdelbegin \DIFdel{, one }\DIFdelend \DIFaddbegin \DIFadd{to translate from one language to another - one from Lustre to OCaml \mbox{%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{found in the work of Motika and von Hanxleden}\DIFaddend , a pattern whose code is primarily designed as a target for code generators \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Motika2015}}\hspace{0pt}%DIFAUXCMD
...
While 82\% of non-C implementations were stackful, only one of the C implementations was stackful\DIFdelbegin \DIFdel{: it can be concluded }\DIFdelend \DIFaddbegin \DIFadd{. We observe that when a language is designed from the ground up to support coroutines, then a stackful implementation is common. On the other hand, such a feature is difficult in C while preserving both backward compatibility and acceptable memory usage. We conclude }\DIFaddend that a stackless implementation is important to C programmers, and this reflects the scarcity of memory resources on the platforms under consideration.
